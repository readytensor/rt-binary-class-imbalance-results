...
This script processes the raw metrics CSV file generated by the 'calculate_metrics.py' script.
...
def prepare_metrics_df_with_metadata(
    ...
    """Prepare metrics df with metadata from models and datasets.
...
def aggregate_metrics(metrics_df: pd.DataFrame, by: str = "overall") -> pd.DataFrame:
    ...
    if by == "overall":
        ...
        groupby = ["Model", "Model Order", "Dataset"]
        metrics_df["Dataset"] = metrics_df["Dataset"].str.replace(
            r"_fold_\d+", "", regex=True
        )
    ...
    grouped = (
        metrics_df.groupby(["Scenario", "Fold"] + groupby)[ordered_metrics].mean().reset_index()
    )

    # Calculate the mean and standard deviation across the 5 folds
    aggregated = (
        grouped.groupby(["Scenario"] + groupby)[ordered_metrics]
        .agg(["mean", "std"])
        .reset_index()
    )
    ...
    aggregated_pivot.rename(columns={"metric": "Metric"}, inplace=True)
...
def pivot_and_order_table(
    ...
    # Remove the temporary 'model_order' column if it was created
